...
class DocumentWC: NSWindowController {
    ...
    var editorVC: EditorVC {
        return (self.contentViewController as! NSSplitViewController).splitViewItems[1].viewController as! EditorVC
    }
    
    @IBAction func toggleNavigator(_ sender: Any) {
        (self.contentViewController as! MainSplitVC).toggleSidebar(nil)
    }
    
    // toolbar item action
    @IBAction func renderContent(_ sender: Any) {
        self.editorVC.renderText()
    }
    
    func export() {
        self.exportPanel.message = "Specify where and how you wish to export..."
        self.exportPanel.nameFieldLabel = "Export As:"
        self.exportPanel.canCreateDirectories = true
        self.exportPanel.isExtensionHidden = false
        self.exportPanel.showsTagField = true
        // get and set accessory view
        let exportAccessoryVC = NSViewController(nibName: "ExportAccessoryView", bundle: mainBundle)
        self.exportPanel.accessoryView = exportAccessoryVC.view
        ...
        self.exportPanel.allowedContentTypes = [UTType(fileTypePopup.item(withTitle: self.selectedExportFileType)!.identifier!.rawValue)!]
        ...
        self.exportPanel.beginSheetModal(for: self.window!) { response in
            if response == .OK {
                ...
                case .pdf:
                    self.editorVC.katexView.createPDF { result in
                        switch result {
                ...
                case .jpeg:
                    break
                case .webArchive:
                    self.editorVC.katexView.createWebArchiveData { result in
                        switch result {
                ...
                case .html:
                    self.editorVC.katexView.evaluateJavaScript("document.documentElement.outerHTML;") { output, _ in
    ...
    @IBAction func insert(_ sender: Any) {
        ...
        let vc = self.editorVC
        
        func insertItem(_ item: String, _ backspace: Int, _ length: Int = 0) {
            let oldCursor = vc.inputTextView.selectedRange()
            vc.inputTextView.insertText(item, replacementRange: oldCursor)
            let newCursor = vc.inputTextView.selectedRange()
            let newRange = NSRange(location: newCursor.location - backspace, length: length)
            vc.inputTextView.setSelectedRange(newRange)
        }
        ...
        func encapsulateItem(_ left: String, _ right: String, _ backspace: Int? = nil, _ forwardspace: Int = 0) {
            let selectedRange = vc.inputTextView.selectedRange()
            var insertRange = NSRange(location: selectedRange.upperBound, length: 0)
            vc.inputTextView.insertText(right, replacementRange: insertRange)
            insertRange = NSRange(location: selectedRange.location, length: 0)
            vc.inputTextView.insertText(left, replacementRange: insertRange)
            ...
            vc.inputTextView.setSelectedRange(insertRange)
        }
