...
/**
 A subclass of `NSHostingController` hosting specifically the `BookmarksPane` view that is used to
 customize certain behaviors.
 
 Adds a click gesture recognizer that sends a command to: highlight the corresponding text ranges
 of the selected bookmark entry, edit the selected bookmark entry, or deselect selected row if the
 user clicked elsewhere in the `List` view.
 
 - Note: This implementation is a workaround for receiving mouse click event on the SwiftUI's `List`
 view. The idea did not seem to be supported by SwiftUI natively as the view modifier
 `onTapGesture(perform:)` did not work gracefully with clear background `List` view, nor did it work
 properly with the rounded-rectangular table cell view.
 */
class BookmarksController: NSHostingController<BookmarksPane>, NSGestureRecognizerDelegate {
    ...
    /**
     ...
     This timer, if `nil`, is registered with a delayed action (of an eligible interval) when a
     mouse click event is received. When a new click event is received, whether or not this timer
     is valid and has a scheduled action will be used to determine if the mouse click event should
     be recognized as part of a double-click sequence. If an existing timer is in place, the timer
     immediately gets invalidated to prevent the registered single-click action from firing and
     perform actions as a result of a double-click event.
     
     - Note: This should always be `nil` when no mouse click event is received in order for it to
     function properly.
     When this is not `nil` as a result of a mouse click event, it should always be regardfully
     reset to `nil` when done.
     */
    ...
    /**
     ...
     repurposed to execute the target action here (hence the action-less gesture recognizer) and
     always prevent the gesture recognizer from proceeding with its state transition, because
