...
extension ErrorPaneVC: TableViewDelegate {
    ...
    func highlightSelectedError(withRow row: Int) {
        editor.reveal(at: entries[row].lineNumber)
    }
    ...
    func tableViewSelectionDidChange(_ notification: Notification) {
        ...
        if (tableView.selectedRow > -1) {
            highlightSelectedError(withRow: tableView.selectedRow)
        }
    }
    ...
    /**
     ...
     `tableViewSelectionDidChange(_:)`, the former is only invoked when the selection changes,
     which does not take into the account the case where the user clicks on the same row.
     */
    func tableView(_ tableView: ATableView, didClick row: Int) {
        highlightSelectedError(withRow: row)
    }
